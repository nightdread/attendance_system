...
**Версия:** 1.4.0 (см. также [теги в Git](#релизы-в-git)).
...
| `terminal` | Страница терминала с QR-кодом (также доступна ролям admin, manager, hr) |
...
| `/terminal` | Терминал с QR-кодом (роли `terminal`, `admin`, `manager`, `hr`) |
...
Подробная инструкция по выпуску релизов: **docs/RELEASES.md** (локальная документация).
